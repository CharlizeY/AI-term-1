...
class Board:
    ...
    def are_ships_too_close(self):
        ...
        Returns:
            bool : return True if and only if there is at least a pair of 
                ...
                otherwise
        """
        is_close_to_all_ships = []
        
        for ship in self.ships:
            remaining_ships = [i for i in self.ships if i != ship]
            # For each ship, check whether there is any ship close to that ship
            is_close_to_one_ship = any(ship.is_near_ship(other_ship) for other_ship in remaining_ships)

        # Summarise the results for all ships
        is_close_to_all_ships.append(is_close_to_one_ship)

        return any(is_close_to_all_ships)
    ...
    def is_attacked_at(self, cell):
        ...
        is_ship_hit = any(ship.receive_damage(cell) for ship in self.ships)
        has_ship_sunk = any(ship.has_sunk() for ship in self.ships)
